...
function PanelHeader({ title, description, badge }) {
  return `<div class="panel-header"><div><h1 id="style-title">${escapeHtml(title)}</h1><p id="style-description">${escapeHtml(description)}</p></div>${badge ? `<span class="badge" id="style-badge">${escapeHtml(badge)}</span>` : ''}</div>`;
}
...
function StylePicker(styles) {
  return `<section class="panel style-picker" aria-labelledby="style-title">
    ...
    ${StyleGrid(styles)}
    <div class="series-workspace" id="series-workspace" hidden>
      <span class="series-workspace__icon">${icon('image')}</span>
      <div class="series-workspace__copy"><h2>画風は基準画像から引き継ぎます</h2><p>被写体、構図、背景は今回の写真を基準にします。今回だけ必要な変更を指定してください。</p></div>
      <button class="button button--primary" id="open-series-settings" type="button">今回の写真を調整</button>
      <dl class="series-workspace__summary"><div><dt>構図・背景</dt><dd id="series-composition-summary">写真全体を描く</dd></div><div><dt>追加指定</dt><dd id="series-instruction-summary">なし</dd></div></dl>
    </div>
  </section>`;
}
...
function AccordionField({ id, label, help, placeholder }) {
  return `<section class="accordion-item">
    <h3><button class="accordion-trigger" type="button" aria-expanded="false" aria-controls="${id}-panel" data-accordion="${id}"><span><strong>${label}</strong><small>${help}</small></span><span class="accordion-trigger__value" id="${id}-summary">未入力</span><span class="accordion-trigger__chevron">${icon('chevron')}</span></button></h3>
    <div class="accordion-panel" id="${id}-panel" hidden><label class="sr-only" for="${id}">${label}</label><textarea id="${id}" placeholder="${placeholder}"></textarea></div>
  </section>`;
}
...
function PhotoSettings() {
  return `<dialog class="settings-panel" id="settings-panel" aria-labelledby="settings-title">
    ...
    <div class="settings-panel__content">
      <section class="settings-group settings-group--composition" aria-labelledby="composition-title"><h3 id="composition-title">構図・背景</h3><fieldset class="choice-list"><legend>構図・背景の扱い</legend>
      ...
      </fieldset><div class="background-field" id="background-row" hidden>${InstructionField({ id: 'background', label: '指定する背景', help: '未入力の場合は白背景', placeholder: '例：白い背景' })}</div></section>
      <section class="settings-group settings-group--instructions" aria-labelledby="instruction-title"><h3 id="instruction-title">必要な項目だけ追加</h3><div class="accordion-list">
        ${AccordionField({ id: 'keep', label: '必ず残すもの', help: '形・個数・模様など', placeholder: '例：取っ手、蓋の花柄' })}
        ${AccordionField({ id: 'remove', label: '取り除くもの', help: 'ラベル・値札・不要な小物など', placeholder: '例：青いラベル' })}
        ${AccordionField({ id: 'change', label: '今回だけの変更', help: '色・位置・大きさなど', placeholder: '例：花の色を青にする' })}
      </div></section>
    </div>
    <div class="settings-panel__footer"><button class="button button--primary settings-panel__done" id="close-settings-done" type="button">完了</button></div>
  </dialog>`;
}
...
export const componentTree = Object.freeze({
  ...
  StylePicker: ['PanelHeader', 'StyleGrid', 'SeriesWorkspace'],
  StyleGrid: ['StyleCard'],
  PhotoSettings: ['CompositionOption', 'InstructionField', 'AccordionField']
});
